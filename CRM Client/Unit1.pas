...
var param:TParam; comp:TComponent; i:integer;
begin
  if isInsertMenuActive then
    SQLGenerator(INSERT_SQL,INSERT,true)
  else
  begin
    OraQuery1.SQL.Text := main_ini.ReadString('SELECTSQL', 'SELECT', '---');
    for i := 0 to self.ComponentCount-1 do
    begin
      if self.Components[i] is TControl then
      begin
        comp := self.Components[i] as TControl;
        param := OraQuery1.Params.FindParam(comp.Name);
      end;
      if (self.components[i] is TEdit) and (param <> nil) then
      begin
        param.Value := (comp as TEdit).Text;
...
var lang:string;
begin
  //lang := ComboBoxLang.Text; uguale a sotto
...
begin
  ...
  begin
    try
      ...
      Application.MessageBox('INTERNAL ERROR 2: Non è stato possibile scaricare il file ini. Contattare l''assistenza.', 'ERROR', 0);
      Halt;
    end;
  end;
...
procedure TGestionaleCartellaClinica.FormDestroy(Sender: TObject);
begin
  freeAndNil(main_ini);
